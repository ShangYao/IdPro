 ...
 * ----生成主页登录后左侧权限的显示标签---------
 */
public class GetSubStrTag extends TagSupport {
	...
	if(!"".equals(n)&&n!=null){	
		ICodeService  codeservice=new CodeServiceImpl();
		  for(int i=0;i<n.length();i++){
			  mdes=mdes+codeservice.getStrByCode(mbcode, code.substring(i, i+1))+",";
		   }
        
				pagestr.append(""+mdes.substring(0,mdes.length()-1)+"");
